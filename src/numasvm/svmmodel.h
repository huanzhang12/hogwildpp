...
struct SVMParams {
  ...
  SVMParams(fp_type stepsize, fp_type stepdecay, fp_type _mu, fp_type beta, fp_type lambda, hazy::thread::ThreadPool * tpool) :
      mu(_mu), step_size(stepsize), step_decay(stepdecay) , beta(beta), lambda(lambda), tpool(tpool) { }
};
